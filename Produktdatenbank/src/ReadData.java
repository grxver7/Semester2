...
public class ReadData {
    //prepare and read Data to create the objects
                ...
                if (line.startsWith("New_Entity: ")) {
                    ...
                    switch (taskID) {
                        case 0 -> {
                            ...
                            ObjectsDB.createPerson(cuttedLine[0], cuttedLine[1], cuttedLine[2]); //personID, personName, person Gender
                        }
                        case 1 -> {
                            cuttedLine = cutterInput(line);
                        ...
                        case 3 -> {
                            cuttedLine = cutterInput(line);
                            ObjectsDB.createCompany(cuttedLine[0], cuttedLine[1]); //companyID, companyName
                        }
                        case 4 -> {
                            cuttedLine = cutterInput(line);
                            ObjectsDB.createProduct(cuttedLine[0], cuttedLine[1]); //productID, productName
                        }
                        case 5 -> {
                            cuttedLine = cutterInput(line);
